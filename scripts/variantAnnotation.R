...
SNV <- data.frame(matrix(ncol = 5, nrow = length(annotated$GENEID)))
colnames(SNV) <- c("Γονίδιο", "Γεννετική Παραλλαγή", "Κλινική σημασία", "Συχνότητα παραλλαγμένου αλληλόμορφου (VAF)", "Συνολική κάλυψη θέσης (coverage)")
